...
describe('ChristmasEmitter', () => {
    ...
    it('EXTENED -should not call callbacks after they have been removed but leave existing ones', () => {
        const emitter = new ChristmasEmitter();
        const mockCallback = jest.fn();
        const mockCallbackNotDeleted = jest.fn()

        emitter.on('letter', mockCallback);
        emitter.on('letter', mockCallbackNotDeleted);
        emitter.off('letter', mockCallback);
        emitter.emit('letter');

        expect(mockCallback).not.toHaveBeenCalled();
        expect(mockCallbackNotDeleted).toHaveBeenCalled();
    });
});